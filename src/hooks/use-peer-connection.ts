...
export function usePeerConnection(options: UsePeerConnectionOptions = {}) {
  ...
  const localDisplayName = options.displayName || "You";
  ...
  useEffect(() => {
    ...
    const broker = new SignalingBroker(peerId, {
      ...
      onRoomJoined: async (data) => {
        ...
        setTimeout(() => {
          if (active) {
            broker.broadcastMediaState(localVideoEnabledRef.current, localAudioEnabledRef.current);
          }
        }, 100);
    ...
    if (autoJoin && initialRoomId) {
      broker.joinRoom(initialRoomId, localDisplayName);
    }
  ...
  }, [peerId, autoJoin, initialRoomId, localDisplayName]);
  ...
  const joinRoom = useCallback(
    (targetRoomId?: string, targetDisplayName?: string) => {
      ...
      setIsRoomFull(false);
      resetAllPeerConnections();
      signalingRef.current?.joinRoom(roomToJoin, targetDisplayName ?? localDisplayName);
    },
    [roomId, localDisplayName, resetAllPeerConnections],
  );
